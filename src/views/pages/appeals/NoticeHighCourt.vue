...
<template>
    <div class="card">
        <!-- Breadcrumb -->
        <Breadcrumb :home="{ icon: 'pi pi-home', to: '/dashboard' }" :model="[{ label: 'Appeals' }, { label: 'Notice Court of Appeal' }]" class="mb-3" />

        <!-- Page Header -->
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">High Court Notices</h1>
                <p class="text-sm text-muted-color mt-1 opacity-70">Manage and track high court notices efficiently</p>
            </div>
            <div class="flex items-center gap-2">
                <span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>
                <span class="text-sm text-muted-color">{{ totalNotices }} notices</span>
            </div>
        </div>
    </div>
