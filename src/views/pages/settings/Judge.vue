...
<template>
    <div class="card">
        <!-- Breadcrumb -->
        <Breadcrumb :home="{ icon: 'pi pi-home', to: '/dashboard' }" :model="[{ label: 'Settings' }, { label: 'Judge' }]" class="mb-3" />

        <!-- Page Header -->
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">Judge Management</h1>
                <p class="text-sm text-muted-color mt-1 opacity-70">Manage tribunal judges and their contact information</p>
            </div>
            <div class="flex items-center gap-2">
                <span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>
                <span class="text-sm text-muted-color">{{ totalJudges }} judges</span>
            </div>
        </div>
    </div>
