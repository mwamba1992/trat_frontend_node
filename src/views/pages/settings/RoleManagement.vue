...
<template>
    <div class="card">
        <!-- Breadcrumb -->
        <Breadcrumb :home="{ icon: 'pi pi-home', to: '/dashboard' }" :model="[{ label: 'Settings' }, { label: 'User Management' }, { label: 'Roles' }]" class="mb-3" />

        <!-- Page Header -->
        <div class="flex justify-between items-start">
            <div>
                <h1 class="text-3xl font-bold text-surface-900 dark:text-surface-0 m-0">Role Management</h1>
                <p class="text-sm text-muted-color mt-1 opacity-70">Define roles and assign permissions to control access</p>
            </div>
            <div class="flex items-center gap-2">
                <span class="inline-block w-2 h-2 rounded-full bg-emerald-500"></span>
                <span class="text-sm text-muted-color">{{ totalRoles }} roles</span>
            </div>
        </div>
    </div>
